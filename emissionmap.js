...
function createMap(totalEmissions) {
  ...
  function createMarkers(response) {
    ...
    let emissionsMarkers = [];

    // Loop through the array.
    for (let index = 0; index < mapData.length; index++) {
        let mapD = mapData[index];

        let emissionMarker = L.marker([mapD.lat, mapD.lon])
        .bindPopup("<h5>" + mapD.country + "<h5><h5>Total CO2 Emissions: " + mapD.total + "</h5>");

        emissionsMarkers.push(emissionMarker)

        console.log(emissionsMarkers)}

        createMap(L.layerGroup(emissionsMarkers));
